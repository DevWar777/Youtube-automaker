...
def concatenate_video(video_paths, output_path):
    ...
    print('vlen 1:', effect_time + EFFECT_DURATION)
    print('vlen 2:', effect_time)
...
def total_speech(SPEECH_PATH):
    ...
    total_audio_length = total_audio.duration
    audio1_length = total_audio.duration
    print('alen 1 :', audio1_length)
    total_audio.close()
...
delta = (audio_final_length - total_audio_length) / len(audio_lengths)
for i in range(len(audio_lengths)):
    audio_lengths[i] -= delta
audio_final_length = sum(audio_lengths)
print('alen 2 :', audio_final_length, len(audio_lengths))

# List images in directory & Calculate parameters
image_paths = os.listdir(IMAGE_PATH)

image_n = len(image_paths)
fps = 1 / ((audio_final_length + (EFFECT_DURATION * (image_n - 1))) / image_n)
